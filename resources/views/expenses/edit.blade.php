...
<div class="max-w-xl mx-auto">
    <h1 class="text-3xl font-bold mb-6">Editar Despesa</h1>
    ...
    <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-6">
        ...
        <div>
            ...
            <input id="name" name="name" type="text" required maxlength="100" value="{{ old('name', $expense->name) }}"
        ...
        <div>
            ...
            <input id="amount" name="amount" type="number" step="0.01" min="0" required value="{{ old('amount', $expense->amount) }}"
                   class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
        </div>

        <div>
            <label for="transaction_date" class="block text-sm font-medium text-gray-700 mb-1">Data</label>
            <input id="transaction_date" name="transaction_date" type="date" required value="{{ old('transaction_date', $expense->transaction_date ? $expense->transaction_date->format('Y-m-d') : null) }}"
                   class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
        </div>
        ...
        <div>
            ...
            <select id="monthly_financial_control_id" name="monthly_financial_control_id" required
                ...
                @foreach($controls as $control)
                    <option value="{{ $control->id }}" {{ old('monthly_financial_control_id', $expense->monthly_financial_control_id) == $control->id ? 'selected' : '' }}>
        ...
        <div class="flex items-center gap-2">
            <input id="fixed" name="fixed" type="checkbox" value="1" {{ old('fixed', $expense->fixed) ? 'checked' : '' }}
