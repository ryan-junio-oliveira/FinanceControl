...
<form action="{{ $action }}" method="POST" class="divide-y divide-gray-100">
    ...
    <div class="p-6 space-y-5">
        <x-form-input name="name" label="Nome" :value="$nameVal" required placeholder="Ex: Mercado, Transporte" />

        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <x-form-input name="amount" label="Valor planejado (R$)" type="number" step="0.01" :value="$amountVal" required placeholder="0,00" />
            <x-form-input name="start_date" label="Data início" type="date" :value="$startVal" required />
            <x-form-input name="end_date" label="Data fim" type="date" :value="$endVal" required />
        </div>
        ...
        <div>
            ...
            <select name="category_id" class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                ...
                @foreach($categories as $c)
                    <option value="{{ $c->id()}}" {{ (string)$categoryVal === (string)$c->id() ? 'selected' : '' }}>{{ $c->name() }}</option>
                @endforeach
            </select>
        </div>
