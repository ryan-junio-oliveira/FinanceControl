...
<div class="max-w-3xl mx-auto">
    ...
    <form action="{{ route('budgets.update', $budget) }}" method="POST" class="form-erp bg-white p-4 rounded-md shadow-sm space-y-4">
        ...
        <div>
            ...
            <x-form-input name="name" :value="old('name', $budget->name)" required />
        </div>

        <div class="grid grid-cols-3 gap-4">
            <div>
                <label class="text-sm text-gray-600">Valor planejado</label>
                <x-form-input name="amount" type="number" step="0.01" :value="old('amount', $budget->amount)" required />
            </div>
            <div>
                <label class="text-sm text-gray-600">Data início</label>
                <x-form-input name="start_date" type="date" :value="old('start_date', $budget->start_date?->format('Y-m-d'))" required />
            </div>
            <div>
                <label class="text-sm text-gray-600">Data fim</label>
                <x-form-input name="end_date" type="date" :value="old('end_date', $budget->end_date?->format('Y-m-d'))" required />
            </div>
        </div>
