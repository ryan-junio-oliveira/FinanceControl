...
@section('content')
    @php
        $breadcrumbs = [['label' => 'Dashboard', 'url' => route('dashboard')], ['label' => 'Orçamentos']];
    @endphp
    <x-list-layout title="Orçamentos" subtitle="Gerencie seus orçamentos" create-url="{{ route('budgets.create') }}"
        ...
        <x-slot name="summary">
            <x-summary-cards>
                <x-card label="Planejado (total)" :value="$budgets->sum(fn($b) => $b->amount())" icon="fa-calendar-days" color="bg-emerald-600" />
            ...
            <x-table compact :columns="$columns" id="budgets-table" tbody-class="bg-white divide-y divide-gray-100">
                @forelse($budgets as $b)
                    <tr class="border-t">
                        ...
                        <td class="px-6 py-4">{{ $b->startDate()->format('d/m/Y') }} —
                            {{ $b->endDate()->format('d/m/Y') }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div
                                ...
                                <x-button variant="ghost" href="{{ route('budgets.show', ['id' => $b->id()]) }}" class="h-8 w-8 rounded-md text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 p-0" aria-label="Ver orçamento {{ $b->name() }}">
                                    <x-fa-icon name="eye" class="h-3.5 w-3.5 text-current" />
                                </x-button>
                                <x-link variant="ghost" href="{{ route('budgets.edit', ['id' => $b->id()]) }}" class="h-8 w-8 rounded-md text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 p-0" aria-label="Editar orçamento {{ $b->name() }}">
                                    <x-fa-icon name="pen" class="h-3.5 w-3.5 text-current" />
                                </x-button>
                                <form action="{{ route('budgets.destroy', ['id' => $b->id()]) }}" method="POST"
                                    onsubmit="return confirm('Remover orçamento?');">
                                    @csrf
                                    @method('DELETE')
                                    <x-button variant="ghost" type="submit" class="h-8 w-8 rounded-md text-gray-400 hover:text-red-600 hover:bg-red-50 p-0" aria-label="Remover orçamento {{ $b->name() }}">
