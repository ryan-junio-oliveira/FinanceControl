...
<div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {{-- Receitas --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4 items-center border-l-4 border-l-emerald-500">
                <div class="w-14 h-14 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-500 text-xl flex-shrink-0">
                    <i class="fa-solid fa-arrow-trend-up"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-widest text-gray-400">Receitas</p>
                    <p class="text-2xl font-bold text-gray-800 mt-0.5 truncate">R$ {{ number_format($totalRecipes ?? 0, 2, ',', '.') }}</p>
                    <p class="text-xs text-gray-400 mt-0.5">Mês atual</p>
                </div>
            </div>

            {{-- Despesas --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4 items-center border-l-4 border-l-red-500">
                <div class="w-14 h-14 rounded-xl bg-red-50 flex items-center justify-center text-red-500 text-xl flex-shrink-0">
                    <i class="fa-solid fa-arrow-trend-down"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-widest text-gray-400">Despesas</p>
                    <p class="text-2xl font-bold text-gray-800 mt-0.5 truncate">R$ {{ number_format($totalExpenses ?? 0, 2, ',', '.') }}</p>
                    <p class="text-xs text-gray-400 mt-0.5">Mês atual</p>
                </div>
            </div>

            {{-- Faturas --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4 items-center border-l-4 border-l-orange-500">
                <div class="w-14 h-14 rounded-xl bg-orange-50 flex items-center justify-center text-orange-500 text-xl flex-shrink-0">
                    <i class="fa-solid fa-credit-card"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-widest text-gray-400">Faturas (Cartões)</p>
                    <p class="text-2xl font-bold text-gray-800 mt-0.5 truncate">R$ {{ number_format($cardsTotal ?? 0, 2, ',', '.') }}</p>
                    <p class="text-xs text-gray-400 mt-0.5">Total das faturas abertas</p>
                </div>
            </div>

            {{-- Investimentos --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4 items-center border-l-4 border-l-violet-500">
                <div class="w-14 h-14 rounded-xl bg-violet-50 flex items-center justify-center text-violet-500 text-xl flex-shrink-0">
                    <i class="fa-solid fa-piggy-bank"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-widest text-gray-400">Investimentos</p>
                    <p class="text-2xl font-bold text-gray-800 mt-0.5 truncate">R$ {{ number_format($totalInvestments ?? 0, 2, ',', '.') }}</p>
                    <p class="text-xs text-gray-400 mt-0.5">Categoria Investimentos</p>
                </div>
            </div>

            {{-- Despesas + Faturas --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4 items-center border-l-4 border-l-rose-600">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-0.5 truncate">R$ {{ number_format(($totalExpenses ?? 0) + ($cardsTotal ?? 0), 2, ',', '.') }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4 items-center border-l-4 {{ $bal >= 0 ? 'border-l-blue-600' : 'border-l-red-600' }}">
                <div class="w-14 h-14 rounded-xl {{ $bal >= 0 ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600' }} flex items-center justify-center text-xl flex-shrink-0">
                    <i class="fa-solid fa-wallet"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-widest text-gray-400">Saldo</p>
                    <p class="text-2xl font-bold mt-0.5 truncate {{ $bal >= 0 ? 'text-blue-700' : 'text-red-600' }}">R$ {{ number_format($bal, 2, ',', '.') }}</p>
                    ...
                    <table class="w-full text-sm text-left text-gray-600">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <td class="py-3 pr-4 text-gray-400">{{ \Carbon\Carbon::parse($tx['date'])->format('d/m/Y') }}</td>
                                <td class="py-3 text-right font-semibold {{ $tx['type'] === 'income' ? 'text-emerald-600' : 'text-rose-600' }}">
...
document.addEventListener('DOMContentLoaded', function () {
                ...
                datasets: [
                    {
                        ...
                        backgroundColor: hex2rgba(GREEN, 0.75), borderColor: GREEN,
                        borderWidth: 1, borderRadius: 4, order: 2,
                    },
                    {
                        label: 'Despesas', data: expenseData,
                        backgroundColor: hex2rgba(RED, 0.75), borderColor: RED,
                        borderWidth: 1, borderRadius: 4, order: 2,
                    },
                    {
                        label: 'Saldo', data: incomeData.map((v,i) => v - (expenseData[i] ?? 0)),
                        type: 'line', borderColor: BLUE, backgroundColor: hex2rgba(BLUE, 0.1),
                        borderWidth: 2, pointRadius: 3, fill: false, tension: 0.35, order: 1,
                    },
                ],
            },
            options: {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    segment: {
                        ...
                        backgroundColor: ctx => ctx.p1.parsed.y < 0 ? hex2rgba(RED, 0.15) : hex2rgba(BLUE, 0.2),
                    },
                }],
            },
            options: {
                ...
                scales: {
                    x: { grid: { display: false }, ticks: { font: { size: 11 } } },
                    y: { ticks: { callback: v => 'R$ ' + v.toLocaleString('pt-BR'), font: { size: 11 } }, grid: { color: 'rgba(0,0,0,0.05)' } },
                },
            },
        });
    }
    ...
    if (ctxExpCat) {
        ...
        const expColors = ['#EF4444','#F97316','#DC2626','#FB923C','#B91C1C','#FDBA74','#991B1B','#FED7AA','#7F1D1D','#FCA5A5'];
        new Chart(ctxExpCat, {
            type: 'doughnut',
            data: { labels: expLabels, datasets: [{ data: expData, backgroundColor: expColors.slice(0, expLabels.length), borderWidth: 2, borderColor: '#fff', hoverOffset: 8 }] },
            options: {
    ...
    if (ctxRecCat) {
        ...
        const recColors = ['#10B981','#059669','#34D399','#047857','#6EE7B7','#065F46','#A7F3D0','#064E3B','#D1FAE5','#022C22'];
        new Chart(ctxRecCat, {
            type: 'doughnut',
            data: { labels: recLabels, datasets: [{ data: recData, backgroundColor: recColors.slice(0, recLabels.length), borderWidth: 2, borderColor: '#fff', hoverOffset: 8 }] },
            options: {
                responsive: true, maintainAspectRatio: false, cutout: '62%',
                plugins: { legend: { position: 'right', labels: { usePointStyle: true, boxWidth: 10, font: { size: 12 } } }, tooltip: { callbacks: { label: ctx => ` ${ctx.label}: ${fmtBRL(ctx.parsed)}` } } },
            },
    ...
    if (ctxCards) {
        ...
        new Chart(ctxCards, {
            ...
            data: {
                labels,
                datasets: [
                    { label: 'Fatura', data: statements, backgroundColor: cardColors,      borderColor: cardColors, borderWidth: 1, borderRadius: 4 },
                    { label: 'Limite', data: limits,     backgroundColor: cardColorsFaint, borderColor: cardColors, borderWidth: 1, borderRadius: 4 },
                ],
            },
            options: {
                responsive: true, maintainAspectRatio: false,
    ...
    if (ctxUtil) {
        ...
        new Chart(ctxUtil, {
            ...
            data: {
                labels: labels2,
                datasets: [{ label: 'Utilizado (%)', data: pctData, backgroundColor: utilColors, borderRadius: 4 }],
            },
            options: {
                indexAxis: 'y',
                responsive: true, maintainAspectRatio: false,
                plugins: {
                    ...
                    tooltip: { callbacks: { label: ctx => ` Utilizado: ${ctx.parsed.x}%` } },
                },
                scales: {
                    x: {
