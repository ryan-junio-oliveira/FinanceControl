...
<div class="max-w-2xl">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <form action="{{ route('credit-cards.update', ['id' => $creditCard->id()]) }}" method="POST" class="divide-y divide-gray-100">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="bank_id" required class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                        @foreach($banks as $b)
                            <option value="{{ $b->id }}" {{ (old('bank_id', $creditCard->bankId()) == $b->id) ? 'selected' : '' }}>{{ $b->name }}</option>
                        @endforeach
                    </select>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <x-form-input name="statement_amount" label="Valor da fatura (R$)" type="number" step="0.01" :value="old('statement_amount', $creditCard->statementAmount())" required />
                    <x-form-input name="limit" label="Limite (opcional)" type="number" step="0.01" :value="old('limit', $creditCard->limit())" />
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <x-form-input name="closing_day" label="Dia de fechamento (1-31)" type="number" min="1" max="31" :value="old('closing_day', $creditCard->closingDay())" />
                    <x-form-input name="due_day" label="Dia de vencimento (1-31)" type="number" min="1" max="31" :value="old('due_day', $creditCard->dueDay())" />
                </div>

                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <x-form-checkbox name="is_active" :checked="old('is_active', $creditCard->isActive())" />
