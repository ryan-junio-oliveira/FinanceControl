...
@section('content')
    <x-list-layout title="Cartões de Crédito" subtitle="Gerencie seus cartões" create-url="{{ route('credit-cards.create') }}"
        create-label="Novo cartão" create-color="bg-cyan-800">
                        ...
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="text-xs text-gray-500 mt-0.5 truncate max-w-[200px]">{{ $card->bank?->name ?? ($card->bank ?? '-') }}</div>
                                </div>
                            </div>
                        </td>

                        <td class="px-6 py-3.5 whitespace-nowrap text-sm text-gray-500">{{ $card->bank?->name ?? $card->bank }}</td>

                        <td class="px-6 py-3.5 whitespace-nowrap text-right">
                            <span class="text-sm font-semibold tabular-nums text-gray-900">R$ {{ number_format($card->statement_amount, 2, ',', '.') }}</span>
                        </td>
                            ...
                            <div class="mx-auto max-w-md">
                                ...
                                <div class="mt-4">
                                    <a href="{{ route('credit-cards.create') }}"
                                        class="inline-flex items-center gap-2 px-4 py-2 rounded bg-cyan-800 text-white text-sm">Novo
                                        cartão</a>
                                </div>
                            </div>
                        </td>
