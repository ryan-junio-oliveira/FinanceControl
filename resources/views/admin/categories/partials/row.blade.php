<tr class="group bg-neutral-primary-soft border-b border-default hover:bg-neutral-secondary-medium transition-colors">
    ...
    <td class="px-6 py-4 whitespace-nowrap">{{ $category->organization?->name }}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ optional($category->created_at)->format('d/m/Y') }}</td>
    <td class="px-6 py-4 whitespace-nowrap text-right">
        <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <x-link variant="ghost" href="{{ route('admin.categories.edit', $category->id) }}" class="inline-flex items-center justify-center rounded-md text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 p-0" aria-label="{{ __('Editar categoria :name', ['name' => $category->name]) }}">
                <x-fa-icon name="pen" class="w-4 h-4 text-current" />
            </x-link>
            <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('{{ __('Remover categoria?') }}');">
                @csrf
                @method('DELETE')
