...
@section('page_title', __('Nova Categoria'))
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST">
        ...
        <div class="space-y-4 p-6">
            <div>
                ...
                <x-form-input id="name" name="name" type="text" value="{{ old('name') }}" required />
            </div>
            <div>
                @php
                    $typeOptions = collect($types)->map(fn($label,$k)=>['value'=>$k,'label'=>$label])->all();
                @endphp
                <x-form-select name="type" label="{{ __('Tipo') }}" :options="$typeOptions" :value="old('type')" required />
            </div>
            <div>
                @php
                    $orgOptions = collect($organizations)->map(fn($org)=>['value'=>$org->id,'label'=>$org->name])->all();
                @endphp
                <x-form-select name="organization_id" label="{{ __('Organização') }}" :options="$orgOptions" :value="old('organization_id')" required />
            </div>
        </div>
